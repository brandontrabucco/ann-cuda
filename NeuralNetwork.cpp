...
NeuralNetwork::NeuralNetwork(vector<int> size, double range, double rate, bool d) {
	...
	for (unsigned int i = 0; i < size.size(); i++) {
		if (i == 0) layers.push_back(InputLayer(size[i], range, d));
		else if (i == (size.size() - 1)) layers.push_back(OutputLayer(size[i], size[i - 1], d));
		else if (i > 0 && (i < (size.size() - 1))) layers.push_back(HiddenLayer(size[i], size[i - 1], d));
	}
}
